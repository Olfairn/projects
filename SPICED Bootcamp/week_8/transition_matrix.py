...
def get_entrance_probabilities():
    """Get probabilities where to go when entering the market"""

    monday = import_data("monday.csv", DATA_FOLDER)
    entrance_prob = monday[monday["customer_no"].duplicated() == False]["location"].value_counts(normalize = True)
    
    return entrance_prob


# Meta-Function to obtain transition probabilities
